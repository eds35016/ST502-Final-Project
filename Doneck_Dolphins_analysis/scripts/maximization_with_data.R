...
for(i in 1 : length(Trier_data$id_game)){
  data_matrix_TS[Trier_data$id_player[i], Trier_data$id_game[i]] <-
    Trier_data$TS_min[i]
}

mean_TS <- apply(data_matrix_TS, FUN = mean, MARGIN = 1, na.rm=TRUE)
...
line_ups$meanTSperformance <- rep(NA, 91)
for(j in 1 : length(line_ups[,1])){
  line_ups$meanTSperformance[j] <- mean_TS[line_ups$X1[j]] + #mean performance of the first player of Lj
    mean_TS[line_ups$X2[j]] + #mean performance of the 2nd player of Lj
    mean_TS[line_ups$X3[j]] + #mean performance of the 3rd player of Lj
    mean_TS[line_ups$X4[j]] + #mean performance of the 4th player of Lj
    mean_TS[line_ups$X5[j]] #mean performance of the 5th player of Lj
}